<template>
<div>
  <ChartTitle>Increase in time spent video gaming during the COVID-19 pandemic in the United States</ChartTitle>
  <div class="chart-body" ref="container">
    ...
    >
      <div v-if="tooltipData">
        <div class="tooltip-item tooltip-title">{{ tooltipData.genre }}</div>
  ...
  computed: {
    ...
    tooltipX() {
      return this.tooltipData !== null 
        ? this.x(this.tooltipData.genre) + this.x.bandwidth() 
        : -999;
    }
  },
  created() {
    json('usvideogame_timespend.json')
    .then(data => {
      this.groups = data.map(d => d.genre);
      this.series = ["total", "pc", "console"];
      this.records = data;
